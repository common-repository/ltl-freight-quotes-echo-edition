...
if (!class_exists('EnEchoLfqLogs')) {
    class EnEchoLfqLogs
    {
        ...
        {
            $obj_classs = new \EnEchoCurl\EnEchoCurl;
            $data = array(
            ...
            $url = EN_ECHO_ROOT_URL_QUOTES . '/request-log/index.php';
            $logs = $obj_classs->en_echo_sent_http_request($url, $data, 'POST');
            ...
            if (isset($logs['severity'], $logs['data']) && $logs['severity'] == 'SUCCESS') {
                ...
                foreach ($logs['data'] as $key => $shipment) {
                    ...
                    $en_address = $address;
                    $class_name = 'tql-log-' . $key . rand(1, 100);
