...
namespace EnEchoLocationAjax;

use EnEchoDistance\EnEchoDistance;
use EnEchoDropshipTemplate\EnEchoDropshipTemplate;
use EnEchoWarehouse\EnEchoWarehouse;
use EnEchoWarehouseTemplate\EnEchoWarehouseTemplate;

if (!class_exists('EnEchoLocationAjax')) {

    class EnEchoLocationAjax
    {

        public function __construct()
        {
            add_action('wp_ajax_nopriv_en_echo_location_save_form_data', [$this, 'en_echo_location_save_form_data']);
            add_action('wp_ajax_en_echo_location_save_form_data', [$this, 'en_echo_location_save_form_data']);

            add_action('wp_ajax_nopriv_en_echo_get_location', [$this, 'en_echo_get_location']);
            add_action('wp_ajax_en_echo_get_location', [$this, 'en_echo_get_location']);

            add_action('wp_ajax_nopriv_en_echo_location_delete_row', [$this, 'en_echo_location_delete_row']);
            add_action('wp_ajax_en_echo_location_delete_row', [$this, 'en_echo_location_delete_row']);
        }

        /**
         * Get response from api when we sent zip code
         */
        public function en_echo_get_location()
        {
            if (isset($_POST['en_echo_location_zip']) && strlen($_POST['en_echo_location_zip']) > 4) {
                $get_address = EnEchoDistance::get_address(sanitize_text_field($_POST['en_echo_location_zip']), 'address');
            ...
            echo wp_json_encode(['message' => EN_ECHO_709]);
            exit;
        }
         ...
         */
        public function en_echo_location_delete_row()
        {
            global $wpdb;
            ...
            if ($location === 'warehouse') {
                $en_location_html = EnEchoWarehouseTemplate::en_load();
                $en_target_location = '.en_location_warehouse_main_div';
            } else if ($location === 'dropship') {
                $en_location_html = EnEchoDropshipTemplate::en_load();
         ...
         */
        public function en_echo_location_save_form_data()
        {
            global $wpdb;
            $post_data = [];
            ...
            if (isset($_POST['en_post_data']) && !empty($_POST['en_post_data'])) {
                ...
                parse_str($en_dollar_post, $post_data);
            }
            ...
            if (isset($post_data['id'], $post_data['location'])) {
                ...
                if ($location === 'warehouse') {
                    ...
                    $en_location_template_obj = new EnEchoWarehouseTemplate();
                    $en_target_location = '.en_location_warehouse_main_div';
                    $validate = ['zip', 'city', 'state', 'country', 'location'];
                } else {
                    $location_step = 'Dropship';
                    $en_location_template_obj = new EnEchoDropshipTemplate();
                    $en_target_location = '.en_location_dropship_main_div';
                ...
                $en_location_data = EnEchoWarehouse::get_data($en_intersected_data);
