...
namespace EnEchoPackage;

use EnEchoDistance\EnEchoDistance;
use EnEchoProductDetail\EnEchoProductDetail;
use EnEchoReceiverAddress\EnEchoReceiverAddress;
use EnEchoWarehouse\EnEchoWarehouse;

/**
 * Get items detail from added product in cart|checkout page.
 * Class EnEchoPackage
 * @package EnEchoPackage
 */
if (!class_exists('EnEchoPackage')) {
    ...
    class EnEchoPackage
    {
        ...
        {
            $product_detail_obj = new EnEchoProductDetail();
            ...
            self::$receiver_address = EnEchoReceiverAddress::get_address();
            if (empty(self::$receiver_address['receiverZip']) || empty(self::$receiver_address['receiverCountryCode'])) {
                return [];
            }

            // Multi-warehouse work when plan enabled
            self::$multi_warehouse = apply_filters("echo_plans_suscription_and_features", 'multi_warehouse');
            ...
            foreach ($package['contents'] as $key => $product) {
                if (isset($product['data'])) {
                    ...
                    $product = apply_filters('en_shippable_handling_units_request', $product, $product, $product_data);
                    $shippable = [];
                    if (isset($product['shippable']) && !empty($product['shippable'])) {
                    ...
                    isset($haz_array[0]) && $haz_array[0] == 'yes' ? $haz_status = 'Y' : $haz_status = 'no';
                    ...
                    $origin_zip_code = '';
                    $shipment_type = EN_ECHO_DECLARED_FALSE;
                    if (!defined('EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS')) {
                        define('EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS', 'no');
                    }
                    // Micro Warehouse
                    ...
                    foreach ($en_product_fields as $en_field_key => $en_custom_product) {
                        ...
                        if (is_callable(array(self::class, $en_function_trigger)) && strlen($is_line_item) > EN_ECHO_DECLARED_ZERO) {
                            ...
                            $suscription_and_features = apply_filters(
                                "echo_plans_suscription_and_features", $is_plans
                            );
                            ...
                            (!is_array($en_location_value) &&
                                strlen($en_location_value) > EN_ECHO_DECLARED_ZERO) ? $product_item[$is_line_item] = $en_location_value : "";
                            ...
                            if (isset($en_location_value['senderZip']) &&
                                ...
                                switch (EN_ECHO_DECLARED_TRUE) {
                                    case $shipping_class == 'ltl_freight':
                                        $shipment_type = EN_ECHO_DECLARED_TRUE;
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        self::$en_request['LTL_FREIGHT'] = EN_ECHO_DECLARED_ONE;
                                        break;
                                    case $shipment_weight > EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS_PRICE &&
                                        EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS == 'yes':
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        $shipment_type = EN_ECHO_DECLARED_TRUE;
                                        $product_weight < EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS_PRICE ?
                                            self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_ECHO_DECLARED_TRUE : "";
                                        break;
                                    case $product_weight < EN_ECHO_SHIPMENT_WEIGHT_EXCEEDS_PRICE:
                                        $shipment_type = EN_ECHO_DECLARED_TRUE;
                    ...
                    if ($shipment_type && strlen($origin_zip_code) > 0) {
                        self::$en_request['product_name'][$origin_zip_code][] = $product_quantity . " x " . esc_attr($product_data->get_title());
        ...
        {
            $reasons = !self::$shipment_type ? array_merge($reasons, [EN_ECHO_712]) : $reasons;
            return (!self::$origin_zip_code > 0) ? array_merge($reasons, [EN_ECHO_713]) : $reasons;
        }
        ...
        static public function en_filter_shipment()
        {
            ...
            self::$en_request = apply_filters("en_woo_addons_carrier_service_quotes_request", self::$en_request, EN_ECHO_SHIPPING_NAME);
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['LTL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['LTL'] = EN_ECHO_DECLARED_TRUE;
        }
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['SMALL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_ECHO_DECLARED_TRUE;
        }
        ...
        static public function en_is_instore_pickup_enabled($location, $sender_location_address)
        {
            ...
            $suscription_and_features = apply_filters(
                "echo_plans_suscription_and_features", 'instore_pickup_local_delivery'
            );
         ...
         */
        static public function en_echo_get_location($location_id = '')
        {
            ...
            $limit = is_array(self::$multi_warehouse) ? true : false;
            $en_location = EnEchoWarehouse::get_data($en_where_clause, $limit);
            // Micro Warehouse
            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                foreach($en_location as $drops_index => $drops) {
                    if(!empty($locations_dropship) && !in_array($drops['id'], $locations_dropship)) {
            ...
            if (!empty($en_location) && is_array($en_location)) {
                if (count($en_location) == 1) {
                    ...
                    $get_address = json_decode(
                        EnEchoDistance::get_address($en_location, $en_access_level, $receiver_address), true);
                    ...
                    switch ($enable_dropship) {
                        case 'yes':
                            ...
                            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                                ...
                                if(!empty($locations_dropship)) {
                                    self::$get_minimum_warehouse = self::en_echo_get_location();
                                    return self::$get_minimum_warehouse;
                                }
                            }
                            $en_echo_get_location = self::en_echo_get_location(get_post_meta(self::$post_id, self::$product_key_name, true));
                            if (empty($en_echo_get_location)) {
                                self::$get_minimum_warehouse = self::en_echo_get_location();
                                return self::$get_minimum_warehouse;
                            }
                            return $en_echo_get_location;
                        default:
                            self::$get_minimum_warehouse = self::en_echo_get_location();
                            return self::$get_minimum_warehouse;
                    }
