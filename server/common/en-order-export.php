...
namespace EnEchoOrderExport;

/**
 * Get and save settings.
 * Class EnEchoOrderExport
 * @package EnEchoOrderExport
 */
if (!class_exists('EnEchoOrderExport')) {
    ...
    class EnEchoOrderExport
    {
        ...
        public function en_orders_sending($orders, $en_orders_ids)
        {
            $post_data = [
                ...
                'licenseKey' => get_option('en_connection_settings_license_key_echo'),
                'currencyUnit' => get_option('woocommerce_currency'),
                'platform' => 'wordpress',
                'orders' => $orders,
            ];

            $response = \EnEchoCurl\EnEchoCurl::en_echo_sent_http_request(EN_ECHO_ORDER_EXPORT_HITTING_URL, $post_data, 'POST', 'Export Order');
